...
- The site is built by `update-models.yml` (cron + on-push when `generate_site.py` changes) and deployed via GitHub Pages from `docs/`.
- Probe data comes from `probe_models.py` via `probe.yml` (cron every 2h).
